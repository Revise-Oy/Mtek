...
class ProductConfigurator(models.TransientModel):
    ...
    def apply_onchange_values(self, values, field_name, field_onchange):
        ...
        field_prefix = self._prefixes.get("field_prefix")
        quantity_prefix = self._prefixes.get("quantity_prefix")
        custom_field_prefix = self._prefixes.get("custom_field_prefix")
        if field_name:
            if field_name.startswith(quantity_prefix):
                dynamic_fields = {k: v for k, v in values.items() if k.startswith(field_prefix)}
                quantity_dynamic_fields = {k: v for k, v in values.items() if k.startswith(quantity_prefix)}

                vals = {}
                vals["quantity"] = quantity_dynamic_fields
                return {"value": vals}
        ...
        for k, v in dynamic_fields.items():
            if len(k.split(quantity_prefix)) < 2:
    ...
    def fields_get(self, allfields=None, write_access=True, attributes=None):
        ...
        for line in attribute_lines:
            ...
            if line.quantity:
                res[quantity_prefix + str(attribute.id)] = dict(
                    default_attrs,
                    type="float",
                    string="Quantity",
    ...
    def prepare_attrs_initial(
        self, attr_lines, field_prefix, quantity_prefix, custom_field_prefix, dynamic_fields, wiz
    ):
        cfg_step_ids = []
        for attr_line in attr_lines:
            ...
            quantity_field = quantity_prefix + str(attribute_id)
        ...
        return attrs, field_name, quantity_field, custom_field, config_steps, cfg_step_ids
    ...
    def add_dynamic_fields(self, res, dynamic_fields, wiz):
        ...
        for attr_line in attr_lines:
            (
                ...
                attr_line, field_prefix, quantity_prefix, custom_field_prefix, dynamic_fields, wiz
            )
            ...
            if attr_line.quantity and quantity_field in dynamic_fields:
                node = etree.Element("field", name=quantity_field, on_change="1", attrs=str(attrs))
                self.setup_modifiers(node)
                xml_dynamic_form.append(node)
    ...
    def read(self, fields=None, load="_classic_read"):
        ...
        quantity_attr_vals = [f for f in fields if f.startswith(quantity_prefix)]
        custom_attr_vals = [f for f in fields if f.startswith(custom_field_prefix)]

        dynamic_fields = attr_vals + custom_attr_vals + quantity_attr_vals
        ...
        for attr_line in self.product_tmpl_id.attribute_line_ids:
            ...
            quantity_field_name = quantity_prefix + str(attr_id)
            ...
            res[0].update({field_name: False, quantity_field_name: False, custom_field_name: False})
            ...
            if self.quantity:
                quantity_vals = ast.literal_eval(self.quantity)
                res[0].update(quantity_vals)
        return res
    ...
    def action_next_step(self):
        ...
        next_step = self.config_session_id.get_next_step(
            ...
            quantity=self.config_session_id.quantity,
